...
def evaluate(do_pred=False, pred_path=None):
    ...
    #  file.write(str(eval_accuracy))
    #  file.write("\n")
    #  file.write("***")
    #  file.write("\n")
    #  file.write(str(eval_loss))
    #  file.write("\n")
    #  file.write('---------------------')
    ...
    pandas.DataFrame({'eval_loss':[float(eval_loss)],'eval_acc':[float(eval_accuracy)]}).to_csv('eval_info.csv',mode='a',header=False)
    return eval_loss, eval_accuracy


if __name__ == "__main__":

    pandas.DataFrame({'eval_loss':[0], 'eval_acc': [0]}).to_csv('eval_info.csv')
    pandas.DataFrame({'train_loss': [0]}).to_csv('train_info.csv')
    args = set_args()
